...
could do once that unit lands. Two numbering schemes share the word "unit" and
collided once — ROADMAP → Findings, 2026-08-03, records the false row it
produced. These ladder units, 01–09, name capabilities. Driving units, the
numbered records in `docs/prompts/`, name work sessions. The last column maps
each row to the driving units that served it; the commit span of every filed
driving-unit record is derived into the status block below by
`scripts/status.sh`, and a driving unit with no filed record shows no span
...
| 05 on-device or server | Keep a job moving with no signal, because the work can run on the phone instead of being sent away. | not started | none yet |
| 06 signals and evaluation | Nothing new to do. The app keeps a record of how jobs went, so a change can be checked against the runs that came before it. | not started | none yet |
| 07 performance and memory | Nothing new to do. How long a job takes and how much memory it uses becomes something anyone can measure. | not started | none yet |
| 08 feature flags | Get a fix or a change without waiting for an App Store update. | not started | none yet |
| 09 entitlements | Paid features can be told apart from free ones. Until then everything is unlocked. | not started | none yet |
...
whole work. Generated by `scripts/status.sh`. Do not edit between
the markers.
<!-- status:end -->
