#!/usr/bin/env bash
#
# plant-ui-in-kit.sh — INV-3, against gate-arch.
#
# Self-contained since 2026-08-04: writes its own planted file instead of
# prepending to a feature file, whose rename had silenced six plants at once.
# The mechanism and the repair are in ROADMAP → Findings. The planted file
# carries exactly one import, so the case's expected count stays exactly 1.

[ -n "${TEETH_WORKTREE:-}" ] || {
    echo 'refusing to plant: TEETH_WORKTREE is not set' >&2
    exit 2
}
_gitdir="$(git -C "$TEETH_WORKTREE" rev-parse --absolute-git-dir 2>/dev/null)" || {
    echo "refusing to plant: $TEETH_WORKTREE is not a git worktree" >&2
    exit 2
}
[ -f "$TEETH_WORKTREE/.teeth-worktree" ] || {
    echo "refusing to plant: $TEETH_WORKTREE carries no teeth marker" >&2
    exit 2
}
_common="$(git -C "$TEETH_WORKTREE" rev-parse --path-format=absolute --git-common-dir 2>/dev/null)"
[ "$_gitdir" != "$_common" ] || {
    echo "refusing to plant: $TEETH_WORKTREE is the main working tree" >&2
    exit 2
}

set -uo pipefail
cd "$TEETH_WORKTREE" || exit 2

FILE=Sources/MidkeepKit/PlantedDefect.swift
[ ! -e "$FILE" ] || {
    echo "plant-ui-in-kit: $FILE already exists" >&2
    exit 2
}

cat > "$FILE" <<'SWIFT'
import SwiftUI

/// Planted by the teeth harness. Never present in a checkout.
enum PlantedImportCarrier {}
SWIFT

[ "$(grep -cE '^import SwiftUI$' "$FILE")" -eq 1 ] || {
    echo 'plant-ui-in-kit: the import did not arrive' >&2
    exit 2
}
[ "$(grep -c '^import ' "$FILE")" -eq 1 ] || {
    echo 'plant-ui-in-kit: expected exactly one import' >&2
    exit 2
}

echo 'planted: MidkeepKit imports SwiftUI'
